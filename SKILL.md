...
- 语音支持 `amr / wav / mp3` 三种输入
- `wav/mp3` 会在本地转为 silk 后再发（对齐 allbot 的发送思路，不依赖 allbot 中转）
- 单条语音最长 59 秒；超过 59 秒会自动切片并按顺序连续发送
- “音乐”按你的约定等价为“语音发送”（提供 `send-music` 子命令）

## 前置条件

- 必须安装 `ffmpeg`（视频封面抽帧、音频解码都会用到），确保命令行可执行 `ffmpeg -version`。
...
说明：
- `amr`：按 AMR 原格式直发。
- `wav/mp3`：先本地转为 silk，再通过 869 的 voice 接口发送。
- 若实际音频时长超过 **59 秒**，脚本会自动拆成多条（每条最多 59 秒）顺序发送。
- `--seconds` 对 `wav/mp3` 主要作为兜底参数；真实发送时长会优先按实际音频长度计算。

返回 JSON 中会额外带上：
- `_derived.inputFormat`：输入格式
- `_derived.wireCodec`：实际发到 869 的编码（`amr` / `silk`）
- `_derived.chunked` / `_derived.chunkIndex` / `_derived.totalChunks`：是否分片及分片序号
